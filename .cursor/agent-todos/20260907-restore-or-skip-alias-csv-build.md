...
1. This code is only on `process_data`, not `main`. Confirm it is OK to implement after that branch is on the default scan/implement branch.
2. Should aliases be generated now, and if so with what rules (the commented stub used `{element}_alias_{n}` placeholders), or should the call site be removed until the etymology algorithm is designed?

## Context

- Primary location: `data_processing/produce_aliases.py` (call site `main.py`).
- Branch: `origin/process_data`.
