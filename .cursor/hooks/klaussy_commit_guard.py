...
# Baked for this Python data repo. No project-wide formatter/linter is configured
# yet (unlike the Node scaffold that used prettier/eslint). Optional `klaussy`
# CLI checks fail open when the binary is missing. Set FORMAT_CMD / LINT_CMD to
# e.g. `ruff format __KLAUSSY_PATHS__` / `ruff check --fix __KLAUSSY_PATHS__`
# once those tools are adopted.
...
def _run(cmd: str) -> int:
    """Run a check with stdout/stderr captured.

    Cursor's beforeShellExecution hook treats anything on this process's stdout
    as a JSON permission decision. Tool chatter (prettier, eslint) on stdout
    therefore blocks the commit as "invalid JSON". Keep our stdout empty; relay
    tool output on stderr so findings still show up when a check fails.

    A missing or unparseable command can't judge the diff, so it allows the
    commit silently rather than block."""
    try:
        tokens = shlex.split(cmd)
    except ValueError:
        return 0
...
def _allow() -> int:
    # Cursor beforeShellExecution expects a JSON decision on stdout.
    print(json.dumps({"permission": "allow"}))
    return 0


def _deny(message: str) -> int:
    print(json.dumps({"permission": "deny", "agent_message": message, "user_message": message}))
...
def main() -> int:
    try:
        ...
        if subjects and not CONVENTIONAL_RE.match(subjects[0]):
            return _deny(_bad_commit_message(subjects[0]))
        ...
        for cmd in (
            ...
            if rc == USAGE_EXIT:
                ...
                continue
            if rc != 0:
                return _deny(_blocked_message(cmd))
        return _allow()
