...
def import_and_process_data() -> pd.DataFrame:
    ...
    if not Path("data/processed/kepn.csv").exists():
        ground_truth_df, elements_df = process_kepn_data(
            csv_folder=Path("data/raw/kepn.csv"),
            output_path=Path("data/processed/kepn.csv"),
        )
